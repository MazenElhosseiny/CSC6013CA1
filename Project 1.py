...
# My name is Mazen Elhosseiny in the CSC6013 course
...
def main():
    ...
    while True:
        ...
        while L.getCurrent() is not None:
            if L.Header.Data > choice_int:
                L.insertBeginning(choice_int)
                print(f"{choice_int} add as the Header to the Linked List")
                L.printList()
                break
            if L.Header.Data == choice_int:
                ans = L.removeBeginning()
                print(f"{ans} removed from the Header of the Linked List")
                L.printList()
                break
            if L.Current.Next is not None and L.getCurrent() < choice_int < L.Current.Next.Data:
                L.insertCurrentNext(choice_int)
                print(f"{choice_int} added to the Linked List")
                L.printList()
                break
            if L.Current.Next is not None and L.Current.Next.Data == choice_int:
                ...
                L.printList()
                break
            if L.Current.Next is None and L.getCurrent() < choice_int:
                L.insertCurrentNext(choice_int)
                print(f"{choice_int} added as the Tail of the Linked list")
                L.printList()
                break
            L.nextCurrent()
